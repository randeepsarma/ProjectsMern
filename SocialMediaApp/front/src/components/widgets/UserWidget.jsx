      ...
      <div className="One h-[20%]">
        <div className="Main h-[99%] flex flex-row">
          <div className="box-border Picture h-full w-[20%] flex justify-center items-center cursor-pointer" onClick={handleProfileJump}>
            {!userData ? <CircularProgress /> : <img src={profileId === obj.user.id ? obj.user.imageUrl : userData?.imageUrl} alt="DP" label="Profile Link" className="h-10 w-10 rounded-full hover:opacity-[0.7]" />}
          </div>
          <div className={`Detail h-full ${profile === "profile" ? "w-[50%]" : "w-[80%]"}   flex flex-col justify-center items-start text-[0.8rem] font-bold`}>
            <div className="Name cursor-pointer hover:underline hover:decoration-[2px]" onClick={handleProfileJump}>{profileId === obj.user.id ? obj.user.firstname : userData?.firstname} {profileId === obj.user.id ? obj.user.lastname : userData?.lastname}</div>
            <div className="Friends cursor-pointer hover:underline hover:decoration-[2px]" onClick={handleProfileJump}>{profileId === obj.user.id ? Object.keys(obj.user.friends).length : userData ? <>{Object.keys(userData?.friends).length} </> : <LinearProgress color="success" />} Friends</div>
          </div>
          <div className={`${profile === "profile" ? "w-[30%] box-border" : "invisible"} flex justify-center items-center `}>

            <Search onClick={handleMessage} >
              {profileId === obj.user.id ? <EditIcon sx={{
                width:'15px',
                height:'15px'
              }} /> : <></>}
