  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await axios({
        url: `http://localhost:5000/user/findwithid/${userId}`,
        method: 'GET',
        headers: {
        ...
        <div className="Initials h-[12vh] flex flex-row  rounded-md box-border " >
          <div className="Image h-full w-[15%] box-border flex justify-center items-center pl-[0.5rem] ">
            <div className="cursor-pointer" onClick={handleProfileJump}>{
              !userInfo ? <CircularProgress /> : <img src={userInfo?.imageUrl} alt="DP" className="h-10 w-10 rounded-full " />
          ...
          <div className="Details h-full w-[70%] flex flex-col justify-center pl-[0.8rem] cursor-pointer" >
            {!userInfo ? <LinearProgress color="success" /> : <p className="text-[0.9rem] font-bold" onClick={handleProfileJump}>{userInfo?.firstname} {userInfo?.lastname}</p>}
